;-------------------------------------------------------------------------------
; lcd_left_to_right - set the text entry as left to right for a Liquid
; Crystal Display with I2C using the PIO and I2C Expansion Boards
...
;-------------------------------------------------------------------------------
; Set text entry as left to right for an I2C Liquid Crystal display
; Registers Used:
;   RF = pointer to display control byte  
; Returns: 
;   DF = 0 on success, DF = 1 on error
;-------------------------------------------------------------------------------
            proc    lcd_left_to_right
            push    rf                ; save pointer register

            load    rf, entry_ctrl    ; get the entry mode control byte
            ldn     rf  
            ori     LCD_L_TO_R        ; set text direction bit (left to right)
