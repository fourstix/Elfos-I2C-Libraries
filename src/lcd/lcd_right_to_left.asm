;-------------------------------------------------------------------------------
; lcd_right_to_left - set the text entry mode as right to left for a
...
;-------------------------------------------------------------------------------
; Set text entry as right to left for an I2C Liquid Crystal display
; Registers Used:
;   RF = pointer to display control byte  
; Returns: 
;   DF = 0 on success, DF = 1 on error
;-------------------------------------------------------------------------------
            proc    lcd_right_to_left
            push    rf                ; save pointer register

            load    rf, entry_ctrl    ; get the entry mode control byte
            ldn     rf  
            ani     LCD_R_TO_L        ; clear text direction bit (right to left)
