...
;-------------------------------------------------------------------------------
; Get the vertical position from device as a 10-bit value.
; Registers Used:
;   RF = pointer to buffer
; Returns: 
;   RD = 10-bit y value (0 to 1023) 
;   DF = 0 on success, DF = 1 on error
;-------------------------------------------------------------------------------

            proc    joystick_get_vertical
            
            push    rf                  ; save buffer pointer
            call    i2c_rdreg           ; i2c address = $20
            db      $20, 1, $05         ; 1 byte register address
            db      2                   ; read 2 bytes from register 5
            dw      joy_y_pos
            lbdf    err_exit            ; if unable to read joystick, exit immediately
            
            load    rf, joy_y_pos       ; get x value from buffer
            lda     rf          
