;-------------------------------------------------------------------------------
; mem_write_data - a routine to write data to an IC2 memory device like 
; an Adafruit Non-Volatile Fram Breakout board or a Sparkfun Qwiic 
; EEPROM Breakout board with I2C using the PIO Expansion Board and I2C 
; adaptor for the 1802/Mini Computer.
;
...
;-------------------------------------------------------------------------------
; This routine writes data to an address in the device.
; Parameters:
...
;   RF points to one past last byte written
;   (RC.0 is consumed)
;-------------------------------------------------------------------------------

            proc    mem_write_data
            
wr_byte:    glo     rc                  ; check to see if more data to read
            lbz     wr_done             ; if not, we're done
            lda     rf                  ; get byte from data buffer
            ...
            lbr     wr_byte             ; keep going until all bytes written  
            
wr_done:    CLC                         ; make sure DF = 0           
err_exit:   return
