;-------------------------------------------------------------------------------
; tsl2561_read_c1 - a function to read the IR sensor value only of the 
...
;-------------------------------------------------------------------------------
; This routine reads the TSL2561 16 bit output of the IR light sensed
; by the i2c device whose address is defined in the tsl2561_def.inc file above
;
; Example: 
;   The following instructs the TSL2561 to read the IR light sensed
;           call    tsl2561_read_c1    ; read sensor value of IR light channel (1)
...
;-------------------------------------------------------------------------------
            ...
            proc    tsl2561_read_c1    ; Read IR channel only
            ...
            mov     r8, read_dest1     ; point to read buffer address
            ...
            db      TSL2561_COMMAND_BIT+TSL2561_REGISTER_CHAN1_LOW+TSL2561_WORD_BIT  ; Register ID
            db      $02                ;           read 2 bytes
read_dest1: dw      $0000              ; read message buffer, A/D low byte, then A/D high byte
