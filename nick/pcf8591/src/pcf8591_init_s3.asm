;-------------------------------------------------------------------------------
; pcf8591_init_s3 - a function to configure the pcf8591 device as 4 discrete
; alalog inputs and outputs, with Channel 3 being active and DAC value from
...
; with analog output from Channel 3. Pcf8591 device address and DAC value are
; defined in the pcf8591_def.inc file above.
;
; Example:
;           call    pcf8591_init_s3    ; initialize device to:
;                                      ; Channel 3 active, 4 discrete inputs, 
...
;-------------------------------------------------------------------------------
            ...
            proc    pcf8591_init_s3    ; a3 --> channel 3
            
            call    i2c_wrbuf          ; write to pcf8591 device, chnl 3 active
            db      I2C_ADDR , $02     ;    where 12c addr = I2C_ADDR for 2 bytes
            dw      pcf8591s3          ; 
            return
            
pcf8591s3:  db    $00+AD_CHNL_3+AD_INP_4SINGLE+AOUT_ENABLE  ; Control Byte:
