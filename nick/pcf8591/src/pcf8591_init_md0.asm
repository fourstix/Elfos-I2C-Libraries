;-------------------------------------------------------------------------------
; pcf8591_init_md0 - a function to configure the pcf8591 device as discrete
; alalog inputs A0, A1 and 1 differential using A2 and A3 inputs. Three outputs 
; Chnl 0, Chnl 1 and  Chnl 2,  with Channel 0 being active and DAC value from
...
; with Channel 0 being active. Pcf8591 device address and DAC value are
; defined in the pcf8591_def.inc file above.
;
; Example:
;           call    pcf8591_init_md0   ; initialize device to:
;                                      ; Channel 0 active, 2 Discrete, 
...
;-------------------------------------------------------------------------------
            ...
            proc    pcf8591_init_md0   ; Mixed Discrete Single: A0 --> Chnl 0
            
            call    i2c_wrbuf          ; write to pcf8591 device, chnl 0 active
            db      I2C_ADDR , $02     ;    where 12c addr = I2C_ADDR for 2 bytes
            dw      pcf8591md0         ; 
            return

pcf8591md0: db    $00+AD_CHNL_0+AD_INP_2S_1DIFF+AOUT_ENABLE  ; Control Byte:
